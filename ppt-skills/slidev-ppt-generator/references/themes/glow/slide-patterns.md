...
Glow 主题专属的组合布局模板，通过 `default` 布局 + UnoCSS + Vue 指令组合实现。

通用参考：
- [Slidev 内置布局参考](../../shared/layout-reference.md) — headmatter 配置、内置布局模板
- [组件参考](../../shared/component-reference.md) — v-click、Link、Toc 等组件用法
- [Glow 主题配置](./theme-config.md) — Glow 专属配置选项和设计约束

---

## 高级布局模板（Glow 主题自定义组合布局）
